...
.text
main:
	la $s0, modulo			# armazena o endereço de memoria de number1 em $a0
	la $s1, numero			# armazena o endereço de memoria de number2 em $a1
...
eh_par: 
	...
	mfhi $v1			# modulo da divisão de t1/2 em $v1
	beqz $v1, imprime_erro		# se o modulo � par, imprime o erro
	jr $ra				# retorna para a funcao que o chamou

eh_primo:
	blt $s0, 2, imprime_erro 	# se o modulo for menor que 2, imprime o erro
	beq $s0, 2, calc_inverso 	# se o numero1 for igual a 2, calcula o inverso pois ele é primo
...
imprime_saida:
	la $a0, string1			# armazena o endereço de memoria de string1 em $a0
	li $v0, 4			# função para imprimir string
	syscall
	j end
    
imprime_erro:
	la $a0, string2			# armazena o endereço de memoria de string2 em $a0
	li $v0, 4			# função para imprimir string
	syscall
